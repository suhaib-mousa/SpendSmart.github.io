...
const downloadBtnRef = useRef(null);
  ...
  useEffect(() => {
    ...
    if (!chartEl || !ctaEl) return;

    const observer = new IntersectionObserver(
      ([entry]) => {
        if (entry.isIntersecting) {
          chartEl.style.position = 'absolute';
          chartEl.style.bottom = '0';
          chartEl.style.top = 'auto';
        } else {
          chartEl.style.position = 'sticky';
          chartEl.style.top = '2rem';
          chartEl.style.bottom = 'auto';
        }
      },
      {
        root: null,
        threshold: 0,
        rootMargin: '0px 0px -100% 0px' // triggers when bottom reaches viewport
      }
    );
  ...
  const downloadPDF = () => {
    ...
    html2pdf()
      ...
      .save()
      .then(() => {
        // Restore the button after rendering
        if (downloadBtnRef.current) {
